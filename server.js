...
server.post("/sendDataLoginAdmin", (req, res) => {
  ...
  function getUserbyLogin(log) {
    return admins.filter(function (admins) {
      return admins.log == log;
    });
  }
  function getUserbyPass(pass) {
    return admins.filter(function (admins) {
      return admins.pass == pass;
    });
  }
  ...
  if (
    adminlog[0].log === login.log &&
    adminpass[0].pass === login.pass &&
    admintoken.token === login.token
  ) {
    res.send({
      msg: "Вы успешно вошли",
      admin: adminlog[0],
